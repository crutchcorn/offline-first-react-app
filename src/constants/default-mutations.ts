...
export function getDefaultMutations(queryClient: QueryClient) {
	...
	queryClient.setMutationDefaults(customerKeys.details().key, {
		...
		): Promise<GetKeyData<typeof customerKeys.details>> => {
			await queryClient.cancelQueries({
				queryKey: customerKeys.detail(person.id).key,
			});
			const { status } =
				(mutationMap.get(
					stabilizeMutationKeys(customerKeys.detail(person.id).key),
				) as GetKeyContext<typeof customerKeys.details> | undefined) ?? {};

			if (status === "conflict") {
				throw new Error(
					"Person has been updated on the server since you last fetched it",
				);
			}
		...
		onMutate: async (person: PersonDetailsInfo) => {
			clearPreviousMutations({
				queryClient,
				mutationKey: customerKeys.detail(person.id).key,
			});
			const serverPersonData = await getPerson({ id: person.id });
			const status =
				serverPersonData.lastUpdated > person.lastUpdated
					? "conflict"
					: "success";
