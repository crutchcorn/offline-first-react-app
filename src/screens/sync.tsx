...
export const Sync = () => {
	const allMutations = useMutationState({
		...
		filters: {
			predicate: (mutation) =>
				mutation.options.mutationKey![0] !== initialDownloadKeys.all().key[0],
		},
	});
...
function PersonListDetails({
	mutation: _mutation,
	index: i,
}: PersonListDetailsProps) {
		...
		switch (mutation.state.status) {
			case "error":
				return (
					...
					>
						Error: {stringify(mutation.state.error)}
					</span>
				);
			case "success":
				return (
